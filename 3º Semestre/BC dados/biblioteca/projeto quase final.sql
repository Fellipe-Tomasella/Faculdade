...
('Carlos Administrador','Bibliotec├írio Chefe','admin','Jyq00N3md314n2mRM9pZEzD6bC/9Z4qdz4slPOiT3Lg='),
...
BEGIN
    ...
    ELSE
        SELECT 'Erro: Livro não disponível para empréstimo (quantidade esgotada).' AS Mensagem;
    END IF;
END$$
DELIMITER ;
...
BEGIN
    ...
    SELECT id_livro, status_emprestimo INTO v_id_livro, v_status_atual
    FROM Emprestimos
    WHERE id_emprestimo = p_id_emprestimo;

    IF v_id_livro IS NULL THEN
        SELECT 'Erro: Empréstimo não encontrado.' AS Mensagem;
    ELSEIF v_status_atual = 'Devolvido' THEN
        SELECT 'Aviso: Este empréstimo já foi devolvido anteriormente.' AS Mensagem;
    ELSE
        START TRANSACTION;
        UPDATE Emprestimos
